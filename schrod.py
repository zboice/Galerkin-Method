...
def initial_func(Xs):
    return -(Xs - a/2 * np.ones_like(Xs))**2 #+ a/2*np.ones_like(Xs) #1/2 * np.sin(1*np.pi*Xs/a) + np.sqrt(3)/2 * np.sin(2*np.pi*Xs/a)
...
ht = 0.01
Xs = np.arange(0, a, h)
times = np.arange(0, 2, ht) # need to watch a CFL-like condition
N_MAX = 4

if ht/h > 1:
    eqn = Galerkin('sines', 1, Xs, N_MAX, times)
