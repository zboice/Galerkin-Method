...
class Galerkin:
    ...
    def basis_init(self, inhomogeneous, off, c0, c1, c2, c_non_lin):
        if self.family == 'sines':
            ...
            self.nstart = 2
            self.bas = lambda n, a, Xs: Galerkin.homogeneous_cheb(n, a, 2*Xs-off*np.ones_like(Xs))
            self.bas_der = lambda n, a, Xs: 2*Galerkin.homogeneous_cheb_deriv(n, a, 2*Xs-off*np.ones_like(Xs))
    ...
    def initial_cvec(self, answer, u0):
        for n in range(self.nstart, self.num):
            answer[n-self.nstart] = simpson_int(self.bas(n, a, self.coord)*u0 * self.norm, h) # issue with norm (weighting function)
        return answer
...
def initial_func(Xs):
    return -(Xs - a/2 * np.ones_like(Xs))**2 + (a/2)**2*np.ones_like(Xs) #1/2 * np.sin(1*np.pi*Xs/a) + np.sqrt(3)/2 * np.sin(2*np.pi*Xs/a)

def compare_initial_info():
    plt.plot(Xs, initial_func(Xs))

    sine_test = Galerkin('sines', None, Xs, 5, None)
    plt.plot(Xs, sine_test.initial_approx(h, Xs))

    cheb_test = Galerkin('homog_cheb', None, Xs, 3, None, off=a)
    plt.plot(Xs, cheb_test.initial_approx(h, Xs))
    # the chebyshev series doesn't agree with the fourier series
    plt.show()
...
def do_animations():
    ...
    if ht/h >= 1:
        # sines, homog_cheb, cheb
        eqn = Galerkin('sines', 1, Xs, N_MAX, times, off=a)
        eqn.runge_kutta(initial_func(Xs))
