...
class Galerkin:
    ...
    def basis_init(self, inhomogeneous, c0, c1, c2, c_non_lin):
        if self.family == 'sines':
            ...
            self.inho = lambda n, t: self.bas(n, a, self.coord) * inhomogeneous #* np.exp(-1j*t)
    ...
    def galerkin(self, t, h, a, N):
        ...
        for n in range(self.nstart, N):
            ...
            f_vec[n-self.nstart] = simpson_int( self.inho(n, t), h)
    ...
    def runge_kutta(self):
        ...
        answer = np.zeros_like(self.coord)
        rvecs = np.zeros([len(int_time), len(answer)], complex)
        rvecs[0] = initial_rvec(self.coord)
        ...
        for j, t in enumerate(self.times[:-1]): # definitely need to do some tweaking here
            # we need the basis for the n+1th time step to be informed by the nth time step?!
            k1 = ht*self.galerkin(t, h, a, N)
            k2 = ht*self.galerkin(t+0.5*ht, h, a, N)
            k3 = ht*self.galerkin(t+0.5*ht, h, a, N)
            k4 = ht*self.galerkin(t+ht, h, a, N)

            '''
            k1 = ht*self.galerkin(t, rvecs[j], w, answer, xs, hx, Nx, gal_num)
            k2 = ht*self.galerkin(t+0.5*ht, rvecs[j]+0.5*k1, w, answer, xs, hx, Nx, gal_num)
            k3 = ht*self.galerkin(t+0.5*ht, rvecs[j]+0.5*k2, w, answer, xs, hx, Nx, gal_num)
            k4 = ht*self.galerkin(t+ht, rvecs[j]+k3, w, answer, xs, hx, Nx, gal_num)
            '''

            rvecs[j+1] = rvecs[j] + (k1+2*k2+2*k3+k4)/6
        
        return rvecs, int_time
...
def initial_rvec(Xs):
    return np.zeros_like(Xs) #-(Xs-0.5*a*np.ones_like(Xs))**2 + (0.5*a*np.ones_like(Xs))**2
...
delta[500:-500] = 1 # this is a really interesting test case, we shouldn't be able to have a "temp" exceeding 1, yet we do

N_MAX = 6
eqn = Galerkin('sines', inhomogeneous(Xs), Xs, N_MAX, times, c2=1, c0=1)
func, times = eqn.runge_kutta()
